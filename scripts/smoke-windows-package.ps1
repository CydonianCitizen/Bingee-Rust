...
1. fresh:   creates <profile>\Bingee Desktop\{data\bingee.db, cache, logs},
            migrates to schema version 3, logs startup, writes nothing to the
...
foreach ($line in @('INFO Bingee Desktop ', 'INFO Migrating the database from schema version 0 to 3', 'INFO Migration to schema version 3 complete', 'INFO Library opened: schema version 3, 0 titles', 'INFO Bingee Desktop closed')) {
...
if (([regex]::Matches($text, 'Library opened: schema version 3')).Count -ne 2) { throw 'second: library not opened.' }
